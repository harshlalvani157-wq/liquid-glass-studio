...
function App() {
  ...
  /* Auto-load preset from public/presets/my-preset.json */
  useEffect(() => {
    const presetPath = '/presets/my-preset.json';
    // Try to fetch the preset file from the public folder
    fetch(presetPath)
      .then((res) => {
      ...
      .then((preset) => {
        // controlsAPI is the Leva API returned by useLevaControls
        if (controlsAPI && typeof controlsAPI === 'function' && preset?.controls) {
          try {
            controlsAPI(preset.controls);
            console.log('Liquid Glass preset applied from', presetPath);
          } catch (err) {
      ...
      .catch((err) => {
        // Not fatal — app will run with default settings
        console.log('No preset applied:', err.message);
      });
  }, [controlsAPI]);
